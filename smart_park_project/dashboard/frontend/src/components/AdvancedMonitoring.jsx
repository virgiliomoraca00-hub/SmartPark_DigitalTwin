...
function TreeCheckbox({ checked, indeterminate, onChange, label, dim = false, count }) {
  return (
    <label className={`flex items-center gap-2 cursor-pointer group select-none ${dim ? 'opacity-50' : ''}`}>
      <span
      ...
      >
        ...
        {!checked && indeterminate && (
          <span className="w-2 h-0.5 bg-emerald-300 rounded" />
        )}
      </span>
      <span className="text-[12px] text-slate-300 group-hover:text-white transition-colors flex-1 truncate">
        {label}
      </span>
      {count != null && (
      ...
      {Object.entries(schema).map(([type, zones]) => {
        ...
        return (
          <div key={type} className="rounded-lg overflow-hidden border border-slate-800/60">
            {/* L1: Tipo ─────────────────────────────────────────────────── */}
            <div
              className="flex items-center gap-2 px-3 py-2 bg-slate-800/50 hover:bg-slate-800 cursor-pointer transition-colors"
...
export default function AdvancedMonitoring({ onClose, sseThings = {} }) {
  ...
  return (
    <div className="fixed inset-0 z-[1100] bg-slate-950/98 backdrop-blur-sm flex flex-col">
      ...
      {/* ── Header ─────────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-6 py-3.5 border-b border-slate-800 bg-slate-900/80 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-black text-white tracking-tight">Monitoraggio Avanzato</h2>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest">
              {stats.things} sensori · {stats.metrics} metriche disponibili · {selection.size} selezioni attive
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-slate-600 uppercase tracking-widest mr-1.5">Intervallo</span>
          {TIME_RANGES.map(tr => (
            <button
              key={tr.value}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg border border-slate-700 overflow-hidden">
            <button
              ...
              className={`px-2.5 py-1.5 text-[11px] font-bold transition-all ${
                panelMode === 'grouped' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'
              }`}
            ...
            <button
              ...
              className={`px-2.5 py-1.5 text-[11px] font-bold transition-all ${
                panelMode === 'single' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'
              }`}
          ...
          <a
            ...
            className="px-2.5 py-1.5 text-[11px] font-bold text-slate-400 hover:text-white border border-slate-700 hover:border-slate-500 rounded-lg transition-all"
          >
            Grafana ↗
          </a>
        ...
        {sidebarOpen && (
          <aside className="w-72 shrink-0 border-r border-slate-800 bg-slate-900/60 flex flex-col overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between shrink-0">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Filtri Sensori</span>
              {selection.size > 0 && (
                <button
                  onClick={clearAll}
          ...
          {selection.size === 0 ? (
            ...
            <div className="space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-slate-600">{panels.length} panel attivi</span>
              </div>
